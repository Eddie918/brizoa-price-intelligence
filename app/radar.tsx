...
export default function Radar() {
  ...
  const [scenario, setScenario] = useState<'now'|'month'|'flexible'>('month');
  ...
  const patch = useCallback(async (id: string, changes: Record<string, unknown>) => {
    if (readOnly) { toast.error('Abre tu espacio privado con ChatGPT para guardar cambios.'); return false; }
    setSaving(true);
    try {
  ...
  useEffect(() => {
    if (loading || readOnly || !active || active.demoKey || active.metadataStatus === 'apify' || attemptedRefresh.current.has(active.id)) return;
  ...
  async function addProduct(e: React.FormEvent) {
    ...
    if (name.trim().length < 2) { setInputError('Escribe un nombre claro para reconocer el producto.'); return; }
    ...
    setSaving(true);
    try {
      const res = await fetch('/api/watchlist', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ input, title: name.trim(), category }) });
  ...
  function updateAmazonInput(value: string) {
    setInput(value); setInputError('');
    const suggestion = titleFromAmazonInput(value);
    if (suggestion && (!nameTouched || !name.trim())) { setName(suggestion); setCategory(guessCategory(suggestion)); }
  }
  function downloadRadar(format: 'csv'|'json') {
    const rows = items.map(item => ({ titulo: item.title, asin: item.externalId, categoria: item.category, estado: item.status, precio_mxn: item.recommendation.current == null ? null : item.recommendation.current / 100, objetivo_mxn: item.targetMinor == null ? null : item.targetMinor / 100, lectura: labels[item.recommendation.action], fuente: item.metadataStatus, actualizado: item.refreshedAt, url: item.url }));
    const content = format === 'json' ? JSON.stringify(rows, null, 2) : '\ufeff' + [['Título','ASIN','Categoría','Estado','Precio MXN','Objetivo MXN','Lectura','Fuente','Actualizado','URL'], ...rows.map(r => [r.titulo,r.asin,r.categoria,r.estado,r.precio_mxn ?? '',r.objetivo_mxn ?? '',r.lectura,r.fuente,r.actualizado ?? '',r.url ?? ''])].map(row => row.map(value => `"${String(value).replaceAll('"','""')}"`).join(',')).join('\n');
    const blob = new Blob([content], { type: format === 'json' ? 'application/json;charset=utf-8' : 'text/csv;charset=utf-8' });
    const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href=url; a.download=`brizoa-radar-${new Date().toISOString().slice(0,10)}.${format}`; a.click(); URL.revokeObjectURL(url);
    toast.success(`Radar descargado en ${format.toUpperCase()}`);
  }
  ...
  const scenarioText = !r || r.current == null ? 'Esperando un precio verificable para comparar escenarios.' : scenario === 'now' ? (r.action === 'buy' ? 'Si lo necesitas ahora, el historial respalda esta compra.' : 'Si es urgente, fija un tope y evita pagar por encima de tu presupuesto.') : scenario === 'month' ? (trend != null && trend < -2 ? 'La tendencia reciente baja; vale la pena observar unos días.' : 'No hay una caída clara. Usa tu objetivo para decidir sin vigilar la tienda.') : `Solo te avisaremos al alcanzar ${active?.targetMinor ? money(active.targetMinor) : 'el precio que tú definas'}.`;
  ...
  return <div className={`app-shell ${dark ? 'theme-dark' : ''}`}>
    ...
    <Dialog open={addOpen} onOpenChange={setAddOpen}><DialogContent className="brizoa-dialog"><DialogHeader><DialogTitle>Algo nuevo en tu radar</DialogTitle><DialogDescription>Pega una URL de Amazon México. Brizoa limpiará el título y consultará una ficha mediante el plan gratuito de Apify cuando esté disponible.</DialogDescription></DialogHeader><form onSubmit={addProduct} className="add-form"><label>URL o ASIN<input autoFocus value={input} onChange={e => updateAmazonInput(e.target.value)} placeholder="https://www.amazon.com.mx/Nombre-del-producto/dp/…" maxLength={2048} required /></label><label>Nombre del producto<input value={name} onChange={e => { setName(e.target.value); setNameTouched(true); }} placeholder="Se completa automáticamente desde la URL" maxLength={150} required />{titleFromAmazonInput(input) && <small className="field-hint"><Sparkles size={13}/> Título sugerido y limpiado desde la URL</small>}</label><label>Categoría<select value={category} onChange={e => setCategory(e.target.value as typeof category)}>{categories.map(c => <option key={c}>{c}</option>)}</select></label><p className="source-note"><ShieldCheck size={16}/> La consulta usa un proveedor externo con caché y límite de 24 horas. Si no hay cuota, la ficha manual seguirá funcionando.</p>{inputError && <p className="form-error" role="alert">{inputError}</p>}<button type="submit" disabled={saving} className="primary-button">{saving ? 'Consultando ficha…' : 'Agregar a mi radar'} <ArrowRight size={17} /></button></form></DialogContent></Dialog>
